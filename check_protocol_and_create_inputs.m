...
function [inputs,vcd] = check_protocol_and_create_inputs(PathNames,out_dir)
...
firstfile = dir(fullfile(PathNames{1},'*.nii'));
if isempty(firstfile) % e.g. empty RF_map folder
    firstfile = dir(fullfile(PathNames{2},'*.nii'));
end
...
%% prepare report file
fid = fopen(fullfile(out_dir,'protocol_check.htm'),'w');
str = ['<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN"' ...
    '"http://www.w3.org/TR/html4/strict.dtd">\n' ...
    '<html>\n' ...
    '<head>\n' ...
    ' <title>NISCI Protocol check results for patient %s</title>\n' ...
    '</head>\n' ...
    '<h1>Protocol check results for folder %s</h1>\n'
    ];
fprintf(fid,str,out_dir,PatID);
...
PName = string(PathNames); % convert to string array
nos = numel(PathNames); % number of apparent sequence folders
...
for prn = 1:nop
    ...
    if contains(lower(curr_seq_name),'b1map') && (vcd == 'p')
        ...
        for nb1 = 1:numel(PName)
            ...
            firstfile = dir(fullfile(PathNames{nb1},'*.nii'));
            firstfile = fullfile(firstfile(1).folder,firstfile(1).name);
        ...
        for cc = 1:nps
            ...
            if isempty(ls)
                fprintf(fid,'Folder <a href="%s">%s</a> of protocol %s is empty.<br>\n', ...
                    cd,strrep(cd,out_dir,''),curr_seq_name);
                continue
            else
                % read in NIFTI files
                mf = dir('*.nii');
            ...
            for fcb1 = 1:numel(nfi)
                if contains(get_metadata_val(nfi{fcb1},'PrepulseType'),'NO')
                    if contains(get_metadata_val(nfi{fcb1},'imtype'),'M_B1')
                        inputs{5,1}{2,1} = char(nfi{fcb1});
                    elseif contains(get_metadata_val(nfi{fcb1},'imtype'),'M_FFE')
                        inputs{5,1}{1,1} = char(nfi{fcb1});
                    end
...
%% write data to files
prot_file_name = fullfile(out_dir,'all_protocol_settings.mat');
save(prot_file_name,'protocol_settings','-v7.3');
inputs_file_name = fullfile(out_dir,'inputs.mat');
